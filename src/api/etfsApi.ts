...
const CACHE_KEY_STOCK = "stockCache";

/**
 * Busca ETFs dos EUA com cache diário no localStorage
 */
...
/**
 * Busca AÇÕES dos EUA com cache diário no localStorage
 */
export async function fetchStocks() {
  const today = new Date().toISOString().slice(0, 10);
  const cachedDataString = localStorage.getItem(CACHE_KEY_STOCK);

  if (cachedDataString) {
    const cached = JSON.parse(cachedDataString);
    if (cached.date === today) {
      console.log("🔁 Usando cache de AÇÕES:", cached.data);
      return cached.data;
    }
  }
  ...
  try {
    ...
    console.log("📡 Dados recebidos da API (AÇÕES):", results);
